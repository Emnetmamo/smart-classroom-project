...
export function ScreenAndRecording({
  mode,
  ...
  useEffect(() => {
    arrivalModeRef.current = arrivalMode;
  }, [arrivalMode]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        log(
          "Smart Screen",
          `Loaded preloaded slides in secure canvas viewer: ${schedule.material?.title ?? "slides"}`,
          "success",
        );
      } catch {
        if (cancelled) return;
  ...
  function buildRecordingName() {
    const cleanCourse =
      (schedule.course || "Lecture").replace(/[^a-z0-9]+/gi, "_").replace(/^_+|_+$/g, "") ||
      "Lecture";
    ...
    const draw = () => {
      if (source.readyState >= 2)
        drawContained(
          ctx,
          source,
          source.videoWidth || RECORDING_WIDTH,
          source.videoHeight || RECORDING_HEIGHT,
        );
    };
    draw();
    recordingDrawTimerRef.current = window.setInterval(draw, 1000 / RECORDING_FPS);
  ...
  async function saveRecordingBlob(
    raw: Blob,
    durationMs: number,
    snap: { sessionId: string; courseId: string; title: string },
  ) {
    if (raw.size < 1024) {
      log("Recording", "Recording too short to save", "warn");
      return;
    }
    const blob = await fixWebmDuration(raw, Math.max(1000, durationMs), { logger: false }).catch(
      () => raw,
    );
    ...
    log(
      "Recording",
      `Saved ${fileName} · ${(blob.size / 1024 / 1024).toFixed(1)} MB — preview and download use the same audio/video file`,
      "success",
    );
  }
  ...
  useEffect(() => {
    if (!teacherPresent || !autoMode || !hasMaterial || pdfStatus !== "ready" || !slideRenderedTick)
      return;
    ...
    (async () => {
      try {
        ...
        try {
          ...
          log("Recording", "Microphone unavailable — recording slides without audio", "warn");
        }
  ...
  const ensureMatcher = useCallback(async () => {
    ...
    try {
      const people: KnownPerson[] = knownTeachers.map((t) => ({
        label: t.id,
        name: t.name,
        imageUrl: t.avatar!,
      }));
      ...
      log(
        "Face Recognition",
        `Ready · ${people.length - failed.length} teacher faces encoded${failed.length ? ` (failed: ${failed.join(", ")})` : ""}`,
      ...
      for (const m of found) {
        ...
        if (hitsRef.current[m.label] === STABLE_HITS) {
          ...
          if (t) {
            log(
              "Face Recognition",
              `Instructor confirmed: ${t.name} (${Math.round((1 - m.distance) * 100)}% match)`,
              "success",
            );
  ...
  async function startManualShare() {
    try {
      ...
      const mime = MediaRecorder.isTypeSupported("video/webm;codecs=vp9,opus")
        ? "video/webm;codecs=vp9,opus"
        : "video/webm";
      const rec = new MediaRecorder(recorderStream, {
        mimeType: mime,
    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Instructor verifies via face on this screen — once recognized, preloaded slides auto-load
          and recording begins. No tab-switching required.
        </p>
      </header>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-center gap-2">
                <div className="inline-flex rounded-md border border-border overflow-hidden text-[11px]">
                  {(["on-time", "warning", "late"] as const).map((m) => {
                    ...
                    const label =
                      m === "on-time" ? "On time" : m === "warning" ? "Warning" : "Late";
                    const tone =
                      m === "on-time"
                        ? active
                    ...
                    return (
                      <button
                        ...
                        className={`px-2 py-1 font-medium ${tone}`}
                        title={`Mark next verification as ${label}`}
                      >
                        {label}
                      </button>
                    );
        ...
        >
                    ...
                    <div className="text-center">
                      ...
                      <div>
                        {modelState === "error"
                          ...
                          : "Start the camera to verify the instructor"}
                      </div>
                      <div className="text-xs mt-1">
                        Sharing & recording start automatically on a match.
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  matches.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {known
                            ? `${name} · ${Math.round((1 - m.distance) * 100)}%`
                            : "Unknown face"}
                        </span>
                      </div>
                    );
            ...
            {!showCam && (
              <>
                ...
                {!liveStream && autoMode && hasMaterial && materialUrl && (
                  <div
                    ref={pdfFrameRef}
                    className="absolute inset-0 bg-background grid place-items-center p-3"
                  >
                    {pdfStatus === "ready" && (
                      <canvas ref={pdfCanvasRef} className="max-w-full max-h-full shadow-lg" />
                    )}
                    {pdfStatus !== "ready" && (
                      <div className="text-center max-w-md text-foreground">
                        {pdfStatus === "loading" ? (
                          <Loader2 className="w-14 h-14 mx-auto text-primary mb-4 animate-spin" />
                        ) : (
                          <FileText className="w-14 h-14 mx-auto text-primary mb-4" />
                        )}
                        <div className="text-xs uppercase tracking-widest opacity-70">
                          {pdfStatus === "error"
                            ? "Slide renderer needs reload"
                            : "Loading preloaded material…"}
                        </div>
                  ...
                  <div className="absolute inset-0 grid place-items-center bg-background/80 p-6">
                    <div className="text-center max-w-sm">
                      ...
                      <div className="text-sm text-muted-foreground mt-1">
                        Recording is armed — share your screen to begin streaming.
                      </div>
                      <button
                        onClick={startManualShare}
                        className="mt-4 px-3 py-2 rounded-md bg-primary text-primary-foreground text-sm inline-flex items-center gap-2"
                      >
          ...
          <div className="mt-4 flex flex-wrap gap-2 items-center">
            ...
            {teacherPresent && teacherLateness && (
              <span
                className={`text-xs inline-flex items-center gap-1.5 px-2 py-1 rounded-full border font-medium ${
            ...
            {!liveStream && teacherPresent && (
              <button
                onClick={startManualShare}
                className="px-3 py-2 rounded-md bg-primary text-primary-foreground text-sm inline-flex items-center gap-2"
              >
                <MonitorPlay className="w-4 h-4" />{" "}
                {hasMaterial ? "Override · share my screen" : "Share my screen"}
              </button>
            )}
            {devices.sharing && (
              <button
                onClick={teacherPresent ? checkOutTeacher : stopMediaTracks}
        ...
        <Panel title="Session metadata">
          <dl className="text-sm space-y-2">
            ...
            <Row
              k="Sharing"
              v={devices.sharing ? (liveStream ? "Live screen" : "Preloaded") : "Off"}
            />
      ...
      {recordedUrl && (
        <Panel title="Recorded preview">
          <video
            controls
            src={recordedUrl}
            className="w-full rounded-lg border border-border max-h-[480px]"
          />
        </Panel>
      )}
    </div>
